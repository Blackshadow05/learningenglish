...
export function validarResumen(valor: unknown, intervenciones: string[] | null): ResumenPractica | null {
  ...
  const citado = (v: unknown): v is string => texto(v) && (intervenciones === null || intervenciones.some(t => normalizar(t).includes(normalizar(v))));
